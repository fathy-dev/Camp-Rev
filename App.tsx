...
import { Campaign, ViewMode, Post, ROLE_PERMISSIONS } from './types';
...
const App: React.FC = () => {
  ...
  // Permission checks
  const canViewPortfolio = hasPermission('canViewPortfolio');
  const canCreatePost = hasPermission('canCreatePost');
  const canEditPost = hasPermission('canEditPost');
  const canDeletePost = hasPermission('canDeletePost');
  const canCreateCampaign = hasPermission('canCreateCampaign');
  const canImportExport = hasPermission('canImportExport');

  // Force Approval mode for viewers who can't view Portfolio
  const effectiveViewMode = canViewPortfolio ? viewMode : 'Approval';
    ...
    <ProtectedRoute>
      <div className={`min-h-screen pb-32 transition-colors duration-500 ${effectiveViewMode === 'Approval' ? 'bg-white' : 'bg-[#F9FAFB]'}`}>
        <Header
          ...
          setViewMode={canViewPortfolio ? setViewMode : () => { }}
        ...
        <main className="max-w-[900px] mx-auto px-6 mt-12 space-y-20">
          <CampaignHero
            ...
            updateCampaign={canEditPost ? updateCampaign : () => { }}
            viewMode={effectiveViewMode}
            onDuplicate={canCreateCampaign ? duplicateCampaign : undefined}
          />
            ...
            {activeCampaign.posts.length > 0 ? (
              activeCampaign.posts.map((post) => (
                <PostCard
                  ...
                  onEdit={canEditPost ? () => setEditingPostId(post.id) : undefined}
                  onDelete={canDeletePost ? () => deletePost(post.id) : undefined}
                  onDuplicate={canCreatePost ? () => duplicatePost(post) : undefined}
                />
              ))
            ) : (
              <div className="text-center py-24 bg-white rounded-[40px] border border-slate-100 shadow-sm flex flex-col items-center">
                ...
                {effectiveViewMode === 'Portfolio' && canCreatePost && (
        ...
        {effectiveViewMode === 'Portfolio' && editingPost && canEditPost && (
          <EditorDrawer
            post={editingPost}
            onUpdate={(data) => updatePost(editingPost.id, data)}
            onClose={() => setEditingPostId(null)}
            onDelete={canDeletePost ? () => deletePost(editingPost.id) : undefined}
          />
        )}
      </div>
    </ProtectedRoute>
